...
SocketInit::Init::Init() {
  ...
  if (error) {
    std::u16string u16errorStr = UTF8_TO_UTF16.from_bytes("Error initializing sockets! Error Code: " + uint_to_str(error, DEC));
...
int TCP_Socket::send(const char* buf, int len) { // envoyer le contenu du buffer
  return ::send(sock, buf, len, 0);
} // SOCKET_ERROR en cas d'erreur, sinon le nbre d'octets envoyés
int TCP_Socket::recv(char* buf, int len) { // lire ce qu'on a reçu sur une longueur max len et le mettre dans buf (bloquant)
...
int TCP_Socket::recv(char* buf, int len, int timeout) { // recv avec timeout (non bloquant ou moins bloquant...) (timeout en ms)
  timeval t;
  fd_set fdsock;
  int ret = 0;
  ...
  if (select(sock + 1, &fdsock, nullptr, nullptr, &t) < 0) { // attend le temps indiqué par timeout
    WSAGetLastError();
    throw SOCKET_EXC; // exception
  }
  if (FD_ISSET(sock, &fdsock)) { // si on a reçu quelque chose sur la socket
...
// TCP_SockServer
TCP_SockServer::TCP_SockServer(int port, int backlog) : TCP_Socket(htonl(INADDR_ANY), port) { // INADDR_ANY... En fait on voudra peut-être préciser l'interface à écouter.. on verra
...
int TCP_SockServer::listen(int backlog) { // écoute les connexions entrantes à cette adresse sur le port précisé. backlog indique le nbre max de connexions en attente.
  return ::listen(*getSock(), backlog);
} // SOCKET_ERROR en cas d'erreur
void TCP_SockServer::accept(TCP_Socket& socket) { // accepter une connexion entrante (bloquant)
...
bool TCP_SockServer::accept(TCP_Socket& socket, int timeout) { // accept avec timeout
  timeval t;
  fd_set fdsock;
  bool ret = false;
...
// NetMessage
NetMessage::NetMessage() : buf(MAXSIZE + 1) { // + 1 pour pouvoir ajouter un /0 après les données reçues
}
bool NetMessage::build(unsigned int id, const void* data, unsigned int datalen) { // construit un message à envoyer
...
bool NetMessage::build(const void* data, unsigned int datalen) { // découpe un message reçu
