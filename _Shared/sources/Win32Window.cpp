...
{
  auto windowIt = std::find_if(winList.begin(), winList.end(),
                               [hWnd](const Win32Window* window)
                               {
...
void Win32Window::init(DWORD dwExStyle, const std::string& classname, const std::string& windowName, DWORD dwStyle, int x, int y, int nWidth, int nHeight, HWND hWndParent, HMENU hMenu, HINSTANCE hInstance, LPVOID lpParam)
{
  ...
  hWnd = CreateWindowEx(dwExStyle, (LPCWSTR)this->wndClass.data(),
                        (LPCWSTR)this->title.data(), dwStyle, x, y, nWidth, nHeight, hWndParent, hMenu, hInstance, lpParam);
}
