...
bool Chess::Move::incrStraight()
{  // fait avancer la position from d'une case vers la position to (en ligne la plus droite possible)
  if (from == to)
    return false;
  int x = (dx > 0 ? 1 : (-1));
...
bool Chess::Move::isMove(const Move& move)
...
bool Chess::Move::isDiag(const Move& move)
{  // déplacement en diagonale
  return abs(move.getDX()) == abs(move.getDY());
}

bool Chess::Move::isLine(const Move& move)
{  // déplacement sur une ligne
  return move.getDY() == 0;
}

bool Chess::Move::isCol(const Move& move)
{  // déplacement sur une colone
  return move.getDX() == 0;
}

bool Chess::Move::isStraight(const Move& move)
{  // déplacement en ligne droite
  return isLine(move) || isCol(move);
}

bool Chess::Move::isAdj(const Move& move)
{  // déplacement sur une case adjacente
  return (abs(move.getDX()) <= 1 && abs(move.getDY()) <= 1);
}

// rêgles de base pour les pièces
bool Chess::Move::isKing(const Move& move)
{  // roi
  return isAdj(move);
}

bool Chess::Move::isKingCastle(const Move& move)
{  // roi roque
  return isLine(move) && abs(move.getDX()) == 2;
}

bool Chess::Move::isQueen(const Move& move)
{  // reine
  return isDiag(move) || isStraight(move);
}

bool Chess::Move::isRook(const Move& move)
{  // tour
  return isStraight(move);
}

bool Chess::Move::isBishop(const Move& move)
{  // fou
  return isDiag(move);
}

bool Chess::Move::isKnight(const Move& move)
{  // cavalier
  return (abs(move.getDX()) == 1 && abs(move.getDY()) == 2) || (abs(move.getDX()) == 2 && abs(move.getDY()) == 1);
}

bool Chess::Move::isPawn(const Move& move, const Color c)
{  // pion déplacement standard
...
bool Chess::Move::isPawnFirst(const Move& move, const Color c)
{  // pion, premier déplacement
  // les pions blancs montent, les noirs descendent (==> pion blanc x diminue, pion noir x augmente)
  return isPawn(move, c) || (isCol(move) && move.getDY() == 2 - (c << 2));
}

bool Chess::Move::isPawnTake(const Move& move, const Color c)
{  // prise du pion (.. déterminer si c'est en passant ou pas)
  return move.getDY() == 1 - (c << 1) && isDiag(move);
}

bool Chess::Move::isPawnEnPassant(const Move& move, const Color c)
{  // suceptible d'être une prise en passant (ça peut aussi etre une prise normale)
...
Chess::EvaluatedMove::EvaluatedMove(const Move& move, int value)
...
Chess::Piece::Piece(int type, int value, Board& board, Color c, Pos pos)
    : type(type), value(value), board(board), color(c), moveCount(0)
{
  setPos(pos);
  resetMoveCount();
}
...
bool Chess::Piece::isLegal(const Move& move) const
{  // vérifie qu'un mouvement est valide pour la pièce
  return Move::isMove(move) && (isKnight() || getBoard().isRightClean(move)) &&
      !getBoard().isDestAlly(move);
}
...
// RookLister
Chess::RookLister::RookLister(int type, int value, Board& board, Color c, Pos pos)
    : Piece(type, value, board, c, pos)
{
}
...
unsigned int Chess::RookLister::listLegalMoves(MoveList& moveList) const
{
  ...
  for (int dx = 0, dy = 1; !getBoard().isDestFoe(move) && move.build(getPos(), dx, dy) &&
       !getBoard().isDestAlly(move);
       ++dy)
  {
    moveList.add(move);
  ...
  // droite
  move.set(getPos(), getPos());
  for (int dx = 1, dy = 0; !getBoard().isDestFoe(move) && move.build(getPos(), dx, dy) &&
       !getBoard().isDestAlly(move);
       ++dx)
  {
    moveList.add(move);
...
// BishopLister
Chess::BishopLister::BishopLister(int type, int value, Board& board, Color c, Pos pos)
    : Piece(type, value, board, c, pos)
{
}
...
unsigned int Chess::BishopLister::listLegalMoves(MoveList& moveList) const
{
  ...
  for (int dx = 1, dy = 1; !getBoard().isDestFoe(move) && move.build(getPos(), dx, dy) &&
       !getBoard().isDestAlly(move);
       ++dx, ++dy)
  {
    moveList.add(move);
...
// King :
Chess::King::King(Board& board, Color c, Pos pos)
    : Piece(IS_KING, 0, board, c, pos)
{
}
...
bool Chess::King::canCastle(const Move& move) const
{
  ...
  return getMoveCount() == 0 && rook && rook->isRook() && rook->getColor() == getColor() && rook->getMoveCount() == 0 &&
      getBoard().isRightClean(rookMove);
...
// Queen :
Chess::Queen::Queen(Board& board, Color c, Pos pos)
    : Piece(IS_QUEEN, 100, board, c, pos), RookLister(IS_QUEEN, 100, board, c, pos), BishopLister(IS_QUEEN, 100, board, c, pos)
{
}
...
// Rook :
Chess::Rook::Rook(Board& board, Color c, Pos pos)
    : Piece(IS_ROOK, 50, board, c, pos), RookLister(IS_ROOK, 50, board, c, pos)
{
}
...
// Bishop :
Chess::Bishop::Bishop(Board& board, Color c, Pos pos)
    : Piece(IS_BISHOP, 30, board, c, pos), BishopLister(IS_BISHOP, 30, board, c, pos)
{
}
...
// Knight :
Chess::Knight::Knight(Board& board, Color c, Pos pos)
    : Piece(IS_KNIGHT, 30, board, c, pos)
{
}
...
// Pawn :
Chess::Pawn::Pawn(Board& board, Color c, Pos pos)
    : Piece(IS_PAWN, 10, board, c, pos)
{
}
...
bool Chess::Pawn::isEnPassant(const Move& move) const
{
  ...
  return isFoe(piece) && piece->isPawn() &&
      piece->getPos() == getBoard().getLastMove().getTo() && piece->getMoveCount() == 1;  // si on trouve un pion adverse qui a avancé de 2 cases au coup d'avant, c'est une prise en passant
}
...
Chess::Piece* Chess::Board::getPiece(const Pos& pos) const
{  // on emploie plutôt operator()(int i) héritée de Matrix, déplacer en private ?
  // TODO : générer exceptions ?
  try
  {
...
bool Chess::Board::isRightClean(Move move) const
{  // vrai si aucune piece n'occupe les cases strictement comprises entre from et to (en ligne droite)
...
Chess::Piece* Chess::Board::getRookCastle(const Move& move) const
{  // renvoie la piece à la position ou devrait se trouver la tour pour un mouvement correspondant à un roque
  Piece* rook = NULL;
...
Chess::Player::Player(const std::string name, const Chess::Color color)
...
const Chess::Player& Chess::Game::initForPlayers(const std::string nameA, const std::string nameB)
{  // initialise les joueurs avec ces 2 noms, renvoie une reférence vers le joueur blanc
  reset();
  if (rand() & 0x0001)
  {  // on décide aléatoirement de qui est blanc, qui est noir
    white.setName(nameA);
    black.setName(nameB);
  }
  else
...
const Chess::Player& Chess::Game::getPlayerFromName(const std::string name) const
{  // on suppose que name est valide
...
bool Chess::Game::isCheck(const Player& player, const Pos& pos)
{  // vrai si au moins une piece adverse menace cette case
...
unsigned int Chess::Game::listLegalMoves(const Player& player, MoveList& moveList)
{
  ...
  for (int i = 0; i < tmpList.length(); ++i)
  {
    ...
    if (!isCastle(move) || isLegalCastle(move))
    {
      ...
      if (IS_CHECK || FROM_POS.getX() == PLAYER_KING_POS.getX() ||
          FROM_POS.getY() == PLAYER_KING_POS.getY() || Move::isDiag(Move(FROM_POS, PLAYER_KING_POS)))
      {
...
bool Chess::Game::isCheckMate(const Player& player)
{  // vrai si en echec et ne peut pas faire de mouvement valide (ou vérifier ça plus "intelligemment" ?)
  return isCheck(player,
                 ((player.getColor() == Chess::C_WHITE) ? whiteKing->getPos() :
                                                          blackKing->getPos())) &&
      !canMakeLegalMove(player);
...
bool Chess::Game::isDraw(const Player& player)
{  // vrai si partie nulle
...
bool Chess::Game::isLegalCastle(const Move& move)
{  // pour vérifier qu'un roque est autorisé (suppose qu'on lui passe un mouvement de roque vérifié)
...
bool Chess::Game::simulateMove(const Move& move, MoveInfo& moveInfo)
{
  ...
  if (isCastle(move))
  {
    ...
    Move rookMove(
        rook->getPos(),
        Pos((((move.getDX()) < 0) ? (move.getTo().getX() + 1) : (move.getTo().getX() - 1)),
            move.getTo().getY()));
  ...
  else if (isEnPassant(move))
  {
    ...
    Piece* pawn = board.getPiece(
        Pos(move.getTo().getX(), move.getTo().getY() + ((pieceFrom->getColor() == Chess::C_WHITE) ? 1 : (-1))));
  ...
  // on peut ensuite effectuer le coup
  if (pieceTo)
  {  // il faut supprimer la piece de destination de la mémoire si ce n'est pas une case vide (on la prend)
    remPiece(pieceTo);
...
int Chess::Game::executeMove(const Move& move)
{  // effectue le mouvement si possible, sinon renvoie un code d'erreur
  MoveInfo moveInfo;
  bool take = false;
  Color c;
  ...
  if (isCheck(getPlayerFrom(c),
              ((c == Chess::C_WHITE) ? whiteKing->getPos() : blackKing->getPos())))
  {
  ...
  for (unsigned int iterativeDeepness = 0; iterativeDeepness < deepness; ++iterativeDeepness)
  {
    ...
    if (iterativeDeepness > 0)
    {
      std::sort(eMoves.begin(), eMoves.end(),
                [](const EvaluatedMove& a, const EvaluatedMove& b) -> bool
                {
    ...
    for (auto& eMove: eMoves)
    {
      ...
      eMove.setValue(mini(iterativeDeepness, alpha, beta, pvLine2Build,
                          (iterativeDeepness > 0) ? &(eMove.getPVLine()) : nullptr, 0));
...
int Chess::Minimax::evaluate(MoveList& moveList)
{
  if (moveList.length() == 0 &&
      game->isCheck(game->getPlayerFrom(game->getTrait()),
                    ((game->getTrait() == Chess::C_WHITE) ? game->whiteKing->getPos() :
                                                            game->blackKing->getPos())))
