...
GuiComponent::GuiComponent(bool enabled)
    : enabled(enabled), window(nullptr), parent(nullptr), hMessWnd(nullptr)
{
  rcLoc = createRC(0, 0, 0, 0);
}
GuiComponent::GuiComponent(const Win32Window* window, RECT rcLoc, bool enabled)
    : enabled(enabled), rcLoc(rcLoc), window(window), parent(nullptr), hMessWnd(nullptr)
{
  if (isEnabled())
    enable();
}
GuiComponent::GuiComponent(const GuiComponent* parent, RECT rcLoc, bool enabled)
    : enabled(enabled), rcLoc(rcLoc), window(nullptr), parent(parent), hMessWnd(nullptr)
{
  if (isEnabled())
    enable();
}
...
RECT GuiComponent::getLocation() const
{  // coordonnées relatives au parent
  return rcLoc;
}
RECT GuiComponent::getClientLoc() const
{  // coordonnées réelles dans la client area
...
GuiPanel::GuiPanel(const Win32Window* window, RECT rcLoc, bool enabled)
    : GuiComponent(window, rcLoc, enabled)
{
  init();
}
GuiPanel::GuiPanel(const GuiComponent* parent, RECT rcLoc, bool enabled)
    : GuiComponent(parent, rcLoc, enabled)
{
  init();
}
GuiPanel::~GuiPanel()
...
{
  HBITMAP hbitmap =
      (HBITMAP)LoadImage(nullptr, (LPCWSTR)UTF8_TO_UTF16.from_bytes(u8filename).data(), IMAGE_BITMAP, 0, 0, LR_DEFAULTSIZE | LR_LOADFROMFILE);
  if (hbitmap == 0)
    return false;
  DeleteObject(hbrush);
...
GuiButton::GuiButton(bool enabled)
    : GuiPanel(enabled), state(STATE_BUTTON), buttonID(0)
{
}
GuiButton::GuiButton(const Win32Window* window, RECT rcLoc, unsigned int buttonID, bool enabled)
    : GuiPanel(window, rcLoc, enabled), state(STATE_BUTTON), buttonID(buttonID)
{
}
GuiButton::GuiButton(const GuiComponent* parent, RECT rcLoc, unsigned int buttonID, bool enabled)
    : GuiPanel(parent, rcLoc, enabled), state(STATE_BUTTON), buttonID(buttonID)
{
}
bool GuiButton::draw(HDC hdc, Gdiplus::Graphics* graphics)
{
  ...
  if (getState() == STATE_CLICKED)
    graphics->DrawImage(&imButtonClicked,
                        Gdiplus::Rect(rcClientLoc.left, rcClientLoc.top,
                                      rcClientLoc.right, rcClientLoc.bottom));
  else if (getState() == STATE_HOVER)
    graphics->DrawImage(&imButtonHover,
                        Gdiplus::Rect(rcClientLoc.left, rcClientLoc.top,
                                      rcClientLoc.right, rcClientLoc.bottom));
  else
    graphics->DrawImage(&imButton,
                        Gdiplus::Rect(rcClientLoc.left, rcClientLoc.top,
                                      rcClientLoc.right, rcClientLoc.bottom));
  if (getState() != STATE_CLICKED)
  ...
  drawCenterText(
      hdc, text,
      createRC(rcClientLoc.left, rcClientLoc.top, rcClientLoc.left + rcClientLoc.right,
               rcClientLoc.top + rcClientLoc.bottom));
  drawEnd(hdc);
  return true;
}
LRESULT GuiButton::eventListener(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
  ...
  else if (message == WM_MOUSEMOVE && pointerIsInside(lParam) && getState() == STATE_BUTTON)
  {  // on survole le bouton et il n'est pas clické ou déjà survolé
    setState(STATE_HOVER);
  }
  else if (message == WM_MOUSEMOVE && !pointerIsInside(lParam) && getState() != STATE_BUTTON)
  {  // le curseur a quitté le boutton alors qu'il était clické ou survolé
    setState(STATE_BUTTON);
  }
  else if (message == WM_LBUTTONDOWN && pointerIsInside(lParam) && getState() != STATE_CLICKED)
  {  // on vient de cliquer sur le bouton
    setState(STATE_CLICKED);
  }
  else if (message == WM_LBUTTONUP && pointerIsInside(lParam) && getState() == STATE_CLICKED)
  {  // le bouton était enfoncé et on a relaché le bouton de la souris en étant dessus (on vient de l'activer)
    setState(STATE_HOVER);
...
// GuiPiece
GuiPiece::GuiPiece(const WCHAR* path, bool enabled, Win32Window* window)
    : GuiComponent(window, createRC(0, 0, 0, 0), enabled), image(path)
{
...
GuiSelectedPiece::GuiSelectedPiece(Win32Window* window, bool enabled)
    : GuiComponent(window, createRC(0, 0, 0, 0), enabled)
{
}
GuiSelectedPiece::GuiSelectedPiece(GuiComponent* parent, bool enabled)
    : GuiComponent(parent, createRC(0, 0, 0, 0), enabled)
{
}
...
bool GuiChessBoard::selectAt(int x, int y)
{
  ...
  if (gpiece)
  {
    ...
    spiece.setLocation(createRC(x - PSIZE / 2 + DECAL, y - PSIZE / 2 + DECAL,
                                gpiece->getLocation().right,
                                gpiece->getLocation().bottom));
    return true;
  }
  return false;
}
...
// public:
GuiChessBoard::GuiChessBoard(bool enabled)
    : GuiComponent(enabled), mboard(BSIZE, BSIZE), spiece(false)
{
  spiece.setParent(this);
}
GuiChessBoard::GuiChessBoard(Win32Window* window, RECT rcLoc, bool enabled)
    : GuiComponent(window, rcLoc, enabled), mboard(BSIZE, BSIZE), spiece(false)
{
  spiece.setParent(this);
}
GuiChessBoard::GuiChessBoard(GuiComponent* parent, RECT rcLoc, bool enabled)
    : GuiComponent(parent, rcLoc, enabled), mboard(BSIZE, BSIZE), spiece(false)
{
  spiece.setParent(this);
}
void GuiChessBoard::enable()
{  // reset aussi les pieces
  // blancs
...
LRESULT GuiChessBoard::eventListener(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
  ...
  else if (message == WM_LBUTTONDOWN && !spiece.isEnabled() &&
           isInsideRect(GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam),
                        createRC(rcClientLoc.left + DECAL, rcClientLoc.top + DECAL,
                                 rcClientLoc.right, rcClientLoc.bottom)))
  {
                                 ...
                                 rcClientLoc.right, rcClientLoc.bottom)))
  {
    ...
    PostMessage(getMessWndHandle(), ThreadMessage::CHECKMOVE,
                translateIndex(spiece.getFrom()), translateIndex(y / PSIZE * BSIZE + x / PSIZE));
  }
  else if (message == WM_MOUSEMOVE && pointerIsInside(lParam) && spiece.isEnabled())
  {
    ...
    RECT rcNew;
    RECT rcOldNew;
    spiece.setLocation(createRC(GET_X_LPARAM(lParam) - rcClientLoc.left - PSIZE / 2,
                                GET_Y_LPARAM(lParam) - rcClientLoc.top - PSIZE / 2,
                                spiece.getLocation().right, spiece.getLocation().bottom));
    ...
    rcOldNew = createRC(rcOld.left < rcNew.left ? rcOld.left : rcNew.left,
                        rcOld.top < rcNew.top ? rcOld.top : rcNew.top,
                        rcOld.right > rcNew.right ? rcOld.right : rcNew.right,
                        rcOld.bottom > rcNew.bottom ? rcOld.bottom : rcNew.bottom);
    InvalidateRect(getWnd()->getHandle(), &rcOldNew, false);
  }
  else if (message == WM_MOUSEMOVE && !pointerIsInside(lParam) && spiece.isEnabled())
  {  // on est sorti de la board, on annule la sélection et remet la pièce à sa place
    cancelSelection();
  }
  return 0;
}
...
{
  if ((r && !reversed) || (!r && reversed))
  {  // alors on doit inverser les éléments dans la matrice (premier devient dernier)
...
void GuiChessBoard::enableSelection()
{  // on autorise une sélection (c'est à notre tour de jouer)
...
// GuiColorInfo
GuiColorInfo::GuiColorInfo(bool enabled)
    : GuiPanel(enabled), timer(0), color(Chess::C_WHITE), side2move(false)
{
}
GuiColorInfo::GuiColorInfo(Win32Window* window, bool enabled)
    : GuiPanel(window, createRC(0, 0, 0, 0), enabled), timer(0), color(Chess::C_WHITE), side2move(false)
{
}
GuiColorInfo::GuiColorInfo(GuiComponent* parent, bool enabled)
    : GuiPanel(parent, createRC(0, 0, 0, 0), enabled), timer(0), color(Chess::C_WHITE), side2move(false)
{
}
...
GuiWaitPanel::GuiWaitPanel(Win32Window* window, bool enabled)
    : GuiPanel(window, createRC(0, 0, 0, 0), enabled)
{
  init();
}
GuiWaitPanel::GuiWaitPanel(GuiComponent* parent, bool enabled)
    : GuiPanel(parent, createRC(0, 0, 0, 0), enabled)
{
  init();
}
bool GuiWaitPanel::draw(HDC hdc, Gdiplus::Graphics* graphics)
{
  ...
  RECT rcClientLoc = getClientLoc();
  drawCenterText(
      hdc, "En attente d'un autre joueur...",
      createRC(rcClientLoc.left, rcClientLoc.top, rcClientLoc.left + rcClientLoc.right,
               rcClientLoc.top + rcClientLoc.bottom));
...
GuiGamePanel::GuiGamePanel(Win32Window* window, bool enabled)
    : GuiPanel(window, createRC(0, 0, 0, 0), enabled)
{
  init();
}
GuiGamePanel::GuiGamePanel(GuiComponent* parent, bool enabled)
    : GuiPanel(parent, createRC(0, 0, 0, 0), enabled)
{
  init();
}
...
GuiMenu::GuiMenu(const Win32Window* window, RECT rcLoc, bool enabled)
    : GuiPanel(window, rcLoc, enabled)
{
}
GuiMenu::GuiMenu(const GuiComponent* parent, RECT rcLoc, bool enabled)
    : GuiPanel(parent, rcLoc, enabled)
{
}
...
GuiMainMenu::GuiMainMenu(const Win32Window* window, RECT rcLoc, bool enabled)
    : GuiMenu(window, rcLoc, enabled)
{
  init();
}
GuiMainMenu::GuiMainMenu(const GuiComponent* parent, RECT rcLoc, bool enabled)
    : GuiMenu(parent, rcLoc, enabled)
{
  init();
}
...
// GuiMainPanel
GuiMainPanel::GuiMainPanel(Win32Window* window, RECT rcLoc, bool enabled)
    : GuiPanel(window, rcLoc, enabled)
{
}
GuiMainPanel::GuiMainPanel(GuiComponent* parent, RECT rcLoc, bool enabled)
    : GuiPanel(parent, rcLoc, enabled)
{
}
...
{
  hLogin = CreateWindowEx(WS_EX_CLIENTEDGE, L"edit", nullptr, WS_CHILD | WS_VISIBLE | WS_TABSTOP, 0, 0, 0, 0,
                          getWnd()->getHandle(),
                          reinterpret_cast<HMENU>(uniqueButtonID.generate()), nullptr, nullptr);
  hPwd = CreateWindowEx(WS_EX_CLIENTEDGE, L"edit", nullptr, WS_CHILD | WS_VISIBLE | WS_TABSTOP | ES_PASSWORD, 0, 0, 0, 0,
                        getWnd()->getHandle(),
                        reinterpret_cast<HMENU>(uniqueButtonID.generate()), nullptr, nullptr);
  MoveWindow(hLogin, getClientLoc().left + 75, getClientLoc().left + 15, 180, 22, true);
  MoveWindow(hPwd, getClientLoc().left + 390, getClientLoc().left + 15, 180, 22, true);
...
// ChessWin
ChessWin::ChessWin(const HINSTANCE hInst, const std::string title, const std::string wndClass, const int width, const int height, const DWORD style)
    : Win32Window(hInst, title, wndClass, width, height, style), hbrush(nullptr), hMessWnd(nullptr)
{
  RECT rcClient;
...
void ChessWin::gamePanelSetPos(bool multi)
{  // en multi on le décale sur le coté, en solo on le centre
  if (multi)
