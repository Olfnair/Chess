...
ClientMain::ClientMain(const HINSTANCE hInst, const std::string wndClass)
    : Win32Window(hInst, "messwnd", wndClass, 0, 0, 0, true), connectionStatus(ConnectionStatus::OFFLINE), menuStatus(MenuStatus::MAIN), window(hInst, "Chess", "Class_CHESS", 1024, 768, WS_OVERLAPPED | WS_VISIBLE | WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX), databuf(NetMessage::MAXSIZE)
{
...
int ClientMain::send(unsigned int id, void* data, unsigned int datalen)
{
  ...
  return threadRecvLoop.getGuest().send(msg.getBuffer().getData(),
                                        msg.getBuffer().getLen());
}
...
bool ClientMain::handleNetMsg()
{
  ...
  if (recvmsg.getID() == NetMsgId::ID && connectionStatus == ConnectionStatus::CONNECTING)
  {  // le serveur nous demande de nous identifier
