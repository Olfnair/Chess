...
void TaskListen::main() {
  try {
    ...
    serverInfo.close();
    while (!isOver()) {
      if (sockServer.accept(guest, 50)) { // si on accepte un client dans les 50 ms :
...
// TaskGuest
TaskGuest::TaskGuest() : connectionStatus(ConnectionStatus::OFFLINE), guest(nullptr) {
}
TaskGuest::TaskGuest(HWND hwnd, const TCP_Socket* sock) : TaskV2(hwnd), sock(*sock), connectionStatus(ConnectionStatus::OFFLINE), guest(nullptr) {
}
...
void TaskGuest::recvLoop(TCP_Socket& sock, NetMessage& msg) {
  ...
  while (!isOver()) {
    ...
    if (timevalDiff(begin, end) > 5000) { // ttes les 5 secs on essaye d'envoyer un message pour voir si l'autre partie n'a pas perdu la connect
      ...
      if (sock.send(msg.getBuffer().getData(), msg.getBuffer().getLen()) <= 0) { // erreur ou fin de connexion
...
// ServerMain
ServerMain::ServerMain(const HINSTANCE hInst, const std::string wndClass) : Win32Window(hInst, "messwnd", wndClass, 0, 0, 0, true), queue(nullptr),
                                                                            window(hInst, "Chess", "Class_CHESS_SERVER", 400, 300, WS_OVERLAPPED | WS_VISIBLE | WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX) {
  window.show(true);
}
...
int ServerMain::mainLoop() { // todo : tester les exceptions
  MSG msg;
...
LRESULT ServerMain::wndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
  if (message == ThreadMessage::NEWCLIENT) {
    ...
    if (!(tg->getCS() == ConnectionStatus::CONNECTED || tg->getCS() == ConnectionStatus::CONNECTING)) { // on vérifie qu'on doit bien écouter les messages de ce client
      return 0;
    }
    handleMessage(tg);
    tg->signal();
...
bool ServerMain::deleteGame(Thread<TaskGuest>* tg) {
  ...
  if (game && (ret = container_erase(games, game))) {
    ...
    if (other && std::find(threadsGuests.begin(), threadsGuests.end(), other) != threadsGuests.end()) { // si other existe encore
...
bool ServerMain::handleMessage(Thread<TaskGuest>* tg) {
  if (tg->getMsg().getID() == NetMsgId::ID && tg->getCS() == ConnectionStatus::CONNECTING) { // le client demande qu'on vérifie ses identifiants
  ...
  } else if (tg->getMsg().getID() == NetMsgId::LOGIN && tg->getCS() == ConnectionStatus::CONNECTING) { // le client envoie son login
  ...
  } else if (tg->getMsg().getID() == NetMsgId::PWD && tg->getCS() == ConnectionStatus::CONNECTING) { // le client envoie son mdp
    tg->getGuest()->setPassword((char*)tg->getMsg().getData());
  } else if (tg->getMsg().getID() == NetMsgId::DISCO && tg->getCS() == ConnectionStatus::CONNECTED) { // le client annonce qu'il va se déconnecter
  ...
  } else if (tg->getMsg().getID() == NetMsgId::GAME_RANDOM && tg->getCS() == ConnectionStatus::CONNECTED) { // demande de partie
    if (!queue)
      queue = tg;
    else {
      ...
      queue->setOther(tg);
      queue->setGame(game);
      Chess::Player white = game->initForPlayers(queue->getGuest()->getLogin(), tg->getGuest()->getLogin());
      int w = Chess::C_WHITE;
      int b = Chess::C_BLACK;
